...
\item \texttt{DONTKNOW}: nel caso in cui, data la mancanza di una visione
  ...
  %  Tale esito è stato introdotto per evitare la presenza di falsi \texttt{MATCH} o falsi \texttt{FAIL}. In tal modo garantiremo all'utente che i  \texttt{MATCH} ed i  \texttt{FAIL} saranno da considerarsi effettivi.
  % questo sta tra i requisiti non-funzionali = qualitiativi..imho..
\end{itemize}
